...
import "./interfaces/ISavManager.sol";
...
contract SavManager is ReentrancyGuard, Governable, ISavManager {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant PRICE_PRECISION = 10 ** 30;
    uint256 public constant USDG_DECIMALS = 18;
    uint256 public constant SAV_PRECISION = 10 ** 18;
    uint256 public constant MAX_COOLDOWN_DURATION = 48 hours;
    uint256 public constant BASIS_POINTS_DIVISOR = 10000;

    IVault public override vault;
    address public override usdg;
    address public override sav;
    ...
    function setCooldownDuration(
        ...
        require(
            ...
            "SavManager: invalid _cooldownDuration"
        );
        cooldownDuration = _cooldownDuration;
    }
    ...
    function addLiquidity(
        ...
        if (inPrivateMode) {
            revert("SavManager: action not enabled");
        }
        return
            _addLiquidity(
                msg.sender,
    ...
    function getPrice(bool _maximise) external view returns (uint256) {
        ...
        uint256 supply = IERC20(sav).totalSupply();
        return aum.mul(SAV_PRECISION).div(supply);
    }
    ...
    function _addLiquidity(
        ...
        require(_amount > 0, "SavManager: invalid _amount");

        // calculate aum before buyUSDG
        uint256 aumInUsdg = getAumInUsdg(true);
        uint256 savSupply = IERC20(sav).totalSupply();
        ...
        require(usdgAmount >= _minUsdg, "SavManager: insufficient USDG output");

        uint256 mintAmount = aumInUsdg == 0
            ? usdgAmount
            : usdgAmount.mul(savSupply).div(aumInUsdg);
        require(mintAmount >= _minSav, "SavManager: insufficient SAV output");

        IMintable(sav).mint(_account, mintAmount);
    ...
    function _removeLiquidity(
        ...
        require(_savAmount > 0, "SavManager: invalid _savAmount");
        require(
            lastAddedAt[_account].add(cooldownDuration) <= block.timestamp,
            "SavManager: cooldown duration not yet passed"
        );
        ...
        uint256 usdgAmount = _savAmount.mul(aumInUsdg).div(savSupply);
    ...
    function _validateHandler() private view {
        require(isHandler[msg.sender], "SavManager: forbidden");
    }
}
