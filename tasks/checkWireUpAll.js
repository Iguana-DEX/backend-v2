...
module.exports = async function (taskArgs) {
  ...
  await Promise.all(
    networks.map(async (network) => {
      ...
      trustedRemoteTable[network] =
        taskArgs.e === "mainnet"
          ? new TrustedRemote()
          : new TrustedRemoteTestnet();
  ...
  for (let i = 0; i < environmentArray.length; i++) {
    ...
    const envToCamelCase = environmentArray[i].replace(/-./g, (m) =>
      m[1].toUpperCase()
    );
    let actualRemoteAddress = getDeployedAddress(
      environmentArray[i],
      taskArgs.proxyChain,
      taskArgs.contract,
    ...
    for (let j = 0; j < environmentArray.length; j++) {
      ...
      let actualLocalAddress = getDeployedAddress(
        environmentArray[j],
        taskArgs.proxyChain,
        taskArgs.contract,
        taskArgs.proxyContract
      ...
      if (actualLocalAddress !== undefined) {
        const currentlySetTrustedRemote =
          trustedRemoteTable[environmentArray[j]][envToCamelCase];
        let actualSetTrustedRemote =
          actualRemoteAddress + actualLocalAddress.substring(2);
        console.log(
          `${environmentArray[j]}'s currentSetRemoteAddress for ${
            environmentArray[i]
          }: ${currentlySetTrustedRemote} ${
            JSON.stringify(actualSetTrustedRemote) ===
            JSON.stringify(currentlySetTrustedRemote)
              ? "✅ "
              : "❌ "
          }`
        );
...
function getDeployedAddress(chain, proxyChain, contract, proxyContract) {
  ...
  try {
    if (chain === proxyChain) {
      deployedAddress =
        getDeploymentAddresses(chain)[proxyContract].toLowerCase();
    } else {
