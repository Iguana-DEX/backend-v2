...
module.exports = async function (taskArgs) {
  ...
  trustedRemoteTable[environment] =
    taskArgs.e === "mainnet" ? new TrustedRemote() : new TrustedRemoteTestnet();
  ...
  await Promise.all(
    environmentArray.map(async (env) => {
      try {
        ...
        trustedRemoteTable[environment][envToCamelCase] =
          await contract.trustedRemoteLookup(dstChainId);
      } catch (error) {
